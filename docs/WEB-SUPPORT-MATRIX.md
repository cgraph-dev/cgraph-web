...
against the live routed web source and focused owner UAT on 2026-05-15.
...
| ---------------------------- | --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Cloud Chat DMs               | `Web partial`         | You can open cloud DMs, send text, send file/photo attachments through the shared upload-first media contract, send voice notes, reply, react, jump from search results, edit/delete/forward messages, handle message requests, pin loaded messages, open a pinned-message panel, see typing, and launch call routes. Stickers/GIFs, guarded autoscroll, read-receipt UI, call-flow verification, and full list management are still incomplete. |
...
| Conversation list management | `Web partial`         | Archive and mark-read actions are mounted on the shipped inbox surface. Pin, mute, mark-unread, archived-list recovery, and per-chat Space moves are still incomplete.                                                                                                                                                                                                                                                                           |
...
| Feature                               | Availability          | User note                                                                                                                                                       |
| ------------------------------------- | --------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Email login and registration          | `Web partial`         | Core routed flows exist, and focused UAT verifies the login route renders. Full end-to-end auth, recovery, 2FA, and edge-state verification remains incomplete. |
| Phone login and registration          | `Web partial`         | Phone entry, OTP, PIN, profile, and permissions steps exist on web.                                                                                             |
| Device verification during phone auth | `Mobile/Desktop only` | The device-attestation checkpoint currently hands off to the mobile/desktop placeholder because the required device keys do not live on web.                    |

...
| Select multiple messages                       | Partial     | The routed Cloud Chat surface now enters select mode from a message action menu or long-press, renders selected-message controls, and browser-verifies batch copy plus batch delete for loaded messages in `apps/web/e2e/dm-media-composer.spec.ts`. Batch forward is not mounted yet.                            | Keep copy/delete proof green and decide whether batch forward is required.         |

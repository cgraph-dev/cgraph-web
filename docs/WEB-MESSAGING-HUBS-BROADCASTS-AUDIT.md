...
| Action                                         | Status      | Source-backed note                                                                                                                                                                                                                                                                                               | Exact web work                                                                     |
| ---------------------------------------------- | ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- |
| Open a cloud DM                                | Ready       | `pages/messages/conversation/page.tsx` gates cloud conversations to the shared `CloudConversation` surface from `modules/chat/components/cloud-conversation`; Vault uses the same owner, and the old page-local enhanced-conversation files are compatibility re-exports.                                                | Keep the canonical routed DM surface proof green while broad live regression continues. |
...
| React to a message                             | Ready       | Backend reaction summaries are normalized into the routed Cloud Chat store without losing `count` / `users`, `MessageBubble` re-renders same-length reaction count/current-user changes, and `apps/web/e2e/dm-media-composer.spec.ts` browser-verifies removing an existing backend-summary reaction plus re-adding it through `/api/v1/messages/:id/reactions`. | Keep the routed add/remove proof green while broader DM regression breadth continues. |
...
| Select multiple messages                       | Ready       | The routed Cloud Chat surface now enters select mode from a message action menu or long-press, renders selected-message controls, and browser-verifies batch copy, batch forward, and batch delete for loaded messages in `apps/web/e2e/dm-media-composer.spec.ts`.                                                   | Keep the batch action proof green while broader DM convergence continues.          |
...
| Record and send voice or video notes           | Ready       | The routed DM composer now mounts voice recording plus camera-backed video-note recording. Voice recordings post to `/api/v1/voice-messages`; video notes upload through the shared message attachment contract and send as `video` messages. Both paths are browser-verified by `apps/web/e2e/dm-media-composer.spec.ts`. | Keep the recorder proofs green while broader DM convergence continues.             |
...
| Open a text channel                                  | Partial | `group-channel.tsx` loads messages, reactions, members, and thread panel; `apps/web/e2e/web-owner-uat.spec.ts` verifies routed text send, direct reply payload/rendering, reaction toggle, thread reply send/rendering, and message actions. Admin/settings and richer media parity remain incomplete. | Finish admin and richer media parity on the live route.                           |
...
| Reply to a channel message               | Ready   | The live channel page exposes reply-to state and reply preview in `message-input.tsx`, sends `reply_to_id`, and renders the returned reply context. Browser-verified by `apps/web/e2e/web-owner-uat.spec.ts`.                                                                        | Keep this path.                                                                   |
| React to a channel message               | Ready   | Reactions are live, the route posts reaction toggles through `/api/v1/messages/:messageId/reactions`, and the optimistic rendered count is browser-verified by `apps/web/e2e/web-owner-uat.spec.ts`.                                                                                 | Add permission edge-state proof.                                                  |
| Open and reply in a thread               | Ready   | `channel-thread-panel.tsx` is real and mounted, thread replies use the same normalized channel-message boundary as main messages, and open/send/render behavior is browser-verified by `apps/web/e2e/web-owner-uat.spec.ts`. Full forum/topic semantics are tracked separately.      | Keep thread replies, then split full forum/topic semantics where needed.          |
...
| Send stickers, GIFs, or voice notes      | Ready   | The routed group composer now mounts sticker and GIF pickers plus `VoiceMessageRecorder`, sends structured `sticker` / `gif` channel messages, uploads channel voice notes through `/api/v1/voice-messages`, and renders the returned media on the live route. Browser-verified by `apps/web/e2e/web-owner-uat.spec.ts`; backend payload proof lives in `apps/backend/test/cgraph_web/controllers/api/v1/channel_message_controller_test.exs`. | Keep the focused proof green while improving progress, retry, and failure UX.     |

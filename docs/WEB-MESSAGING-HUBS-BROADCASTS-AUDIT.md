...
- The DM call route is only partially trusted. `apps/web/e2e/web-owner-uat.spec.ts` now proves the
  route mounts controls, but the web call flow still needs full media/incoming/end-state browser
...
| Global search and jump to message      | Partial | `messages.tsx` opens `MessageSearch` and navigates with `?scrollTo=...`, and `useEnhancedConversation.ts` now consumes that query param to scroll the target message into view. Guarded scroll and unread-jump behavior are still missing.                                                                           | Keep the jump path, then add guarded scroll and unread anchors.                |
...
| Send stickers or GIFs                          | Missing     | No routed picker or send path exists.                                                                                                                                                                                                                                    | Port sticker/GIF pickers into the routed surface.                         |
| Emit typing while typing                       | Partial     | `useEnhancedConversation.ts` now emits `typing=true` from input changes and clears typing on timeout or send, but the routed DM path still needs browser verification end-to-end.                                                                                        | Browser-verify the live typing flow and keep it in the routed input path. |
| Display incoming typing                        | Partial     | `TypingIndicator` renders when store typing state changes.                                                                                                                                                                                                               | Keep it, but pair it with real typing emit.                               |
| Search within a DM and jump to result          | Ready       | Search opens `/messages/:conversationId?scrollTo=:messageId`, and the routed conversation consumes that anchor; browser-verified by `apps/web/e2e/dm-media-composer.spec.ts`.                                                                                            | Keep guarded autoscroll tracked separately.                               |
...
| Guard autoscroll and jump to unread            | Missing     | The route still scrolls to bottom on message-count change.                                                                                                                                                                                                               | Add unread anchors and guarded autoscroll.                                |
| Launch voice or video calls from the DM header | Partial     | `conversation-header.tsx` now routes through `getDirectCallRoute(...)`, but the end-to-end web call flow is not fully trusted yet.                                                                                                                                       | Browser-verify outgoing, incoming, and history-based call entry.          |
...
| React to a channel message               | Ready   | Reactions are live, and the routed message menu now covers edit, delete, report, copy-link, and pin entry actions.                                                                                                                                                 | Browser-verify menu actions with permission edge states.                            |
...
| Attach files or images                   | Partial | `message-input.tsx` has file picker UI, and `group-channel.tsx` now uploads through `/api/v1/uploads` before sending the shared upload-first attachment payload through `sendChannelMessage(...)`. Group browser verification and richer media parity remain open. | Browser-verify the group attachment path and keep the shared contract.              |
...
| Search messages in a channel             | Partial | The routed header now opens real loaded-channel message search, jumps between matches, highlights the target message, and consumes `?scrollTo=...` links. It is not yet a backend older-history search.                                                            | Browser-verify the jump path and decide whether older-message search is required.   |
| Change notifications or mute the channel | Partial | The routed header now toggles the current member's group notification level through `PATCH /api/v1/groups/:groupId/members/me/notifications`. It is group-level, not per-channel granular.                                                                         | Browser-verify mute/unmute and decide whether channel-level preferences are needed. |
| Open pinned messages list                | Partial | `pinned-messages-panel.tsx` can fetch and unpin pins, and the live message menu can create pins through the channel pins endpoint.                                                                                                                                 | Browser-verify pin creation, unpin, and permission-denied states.                   |
| Pin a message from the live message UI   | Partial | The routed message menu now creates pins through `/api/v1/groups/:groupId/channels/:channelId/pins`; unpin remains owned by the pinned panel.                                                                                                                      | Browser-verify pin creation and decide if inline unpin is needed.                   |
...
9. Finish route-owned message parity: report, guarded unread/jump behavior, remaining delivery-state
   UI, and any message-state controls beyond the now-routed read receipts.
10. Finish route-owned call parity: keep the routed DM call buttons and verify outgoing, incoming,
    and history-based call entry in the browser.

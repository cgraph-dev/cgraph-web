...
6. Call history trustworthiness is now route/contract/test aligned. The page fails honestly on API
   ...
   candidate exchange, remote streams, and connected-state handoff; the remaining gap is a
   production-like two-client WebRTC browser session across real network conditions.
...
- [x] Verify local WebRTC signaling mechanics for backend ICE servers, local tracks, offers,
      answers, ICE candidates, remote streams, and connected-state handoff.

...
| Routed group parity                              | `57%` | Group fetch/send works after landing on a valid channel, routed text and file/photo sends are browser-verified, voice/video channel routes now mount a LiveKit-backed group room view, group settings/admin tabs are mounted, routed loaded-message search and group-level mute/unmute are browser-verified, and channel message menus now wire edit/delete/report/pin/copy-link actions. Remaining gaps are permission edge states, older-message search coverage, browser proof for message actions/admin paths, channel-level notification semantics, and voice/sticker/GIF media parity.                                                                                                                   |
...
Equal-weight release-readiness score: `60.8%`.

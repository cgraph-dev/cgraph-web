...
- Nodes false-success handling is now materially fixed in targeted tests; routed wallet/shop browser
  ...
  now browser-verified; real hosted payment completion/webhook settlement and final broad validation
  remain open.
...
- Nodes wallet, shop, tip, gift, unlock, and checkout now throw or render explicit failure states in
  ...
  browser-verified; real hosted checkout completion/webhook settlement and broader final release
...
| Call launch coverage                                | Partial | The manual `/call/:recipientId/:callType` route, routed DM header launch, call-history callback, and incoming-call modal accept into `/call/:recipientId/:callType?incoming=true&roomId=...` are browser-verified. The incoming path also proves visible video controls and end-call navigation back to the DM route. Focused WebRTC tests now prove backend-provided ICE servers, local-track attachment, outbound offers, inbound offer answers, remote answers, ICE candidates, remote streams, connected-state handoff, and a linked two-peer caller/callee signaling sequence through the shared signaling owner. Remaining risk is a production-like two-client WebRTC browser session across real network conditions.                                                                                                                                                                                                                                            |
...
The remaining release risk is real hosted Stripe payment completion/webhook settlement and the
...
4. Routed DM call launch is now proven from the live header, and incoming-call accept now hands off
   ...
   covers caller/callee offer, answer, ICE, remote stream, and connected-state handoff. Deeper peer
   media negotiation still needs browser verification against a production-like WebRTC session.
...
6. Call history trustworthiness is now route/contract/test aligned. The page fails honestly on API
   ...
   candidate exchange, remote streams, connected-state handoff, and a linked caller/callee signaling
   sequence; the remaining gap is a production-like two-client WebRTC browser session across real
   network conditions.
...
| Final release validation                         | `54%` | The full local release-gates are green after the routed-placeholder sweep, `apps/web/src/routes/__tests__/route-inventory.test.ts` guards against reintroducing literal coming-soon routed panels, and 2026-05-28 production web commit `01f55bf` passed a rebuilt broad owner UAT route pass for auth entry, DMs, groups/settings, social, settings/privacy, Nodes, calls, call history, and voice rooms via `apps/web/e2e/web-owner-uat.spec.ts`. Vercel deployed the commit successfully and `pnpm --filter @cgraph/web smoke:production` passed against `https://web.cgraph.org` and `https://cgraph-backend-prod-v2.fly.dev`. A later rebuilt Chromium slice passed 40 / 40 tests across `apps/web/e2e/auth-account-routes.spec.ts`, `apps/web/e2e/dm-media-composer.spec.ts`, `apps/web/e2e/social-main-pane.spec.ts`, `apps/web/e2e/settings-preference-sync.spec.ts`, and `apps/web/e2e/nodes-wallet-shop.spec.ts` after keeping E2E auth bypass from hard-failing explicit login actions. The same rebuilt app then passed 46 / 46 group/forum route tests plus `apps/web/e2e/web-owner-uat.spec.ts`, replaying mounted group settings, invite, scroll, forum, incoming-call accept/end, manual call, call-history callback, and voice-room flows. The Nodes shop route now also has successful Stripe handoff proof, separate browser-profile settings sync proof, and current 2026-05-28 production-web Vitest passes of 397 / 397 files and 5,347 / 5,347 tests after moving Cloud Chat controller ownership into `modules/chat`, adding shared test contracts for onboarding status and conversation media, splitting forum list/detail/moderation hooks into synchronized slice-owned stores, then quieting targeted async-effect tests for settings storage, notification settings, edit history, report submission, emoji picker, automod settings, and LiveKit auto-connect. Production backend commit `4110c7f75b0124bf62f2c60caae40d68600a62ee` also restores the backend `k6/load.js` matrix with real upload, group, sustained-search, and broadcast-publish scenarios; every `k6/*.js` file passed `node --check`, every scenario passed `k6 inspect`, and the broadcast route contract passed 17 focused backend tests. This is not the full strict per-suite release signoff yet: the remaining validation work is external provider checks, paired QR/mobile approval, physical cross-device sync, real hosted Stripe payment completion/webhook settlement, and deeper production-like two-client media/call negotiation. |

Equal-weight release-readiness score: `81.2%`.

Truthful rounded Goal 2 completion: `81%`.

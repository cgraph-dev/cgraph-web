...
- Nodes false-success handling is now materially fixed in targeted tests; routed wallet/shop browser
  ...
  insufficient-balance, already-unlocked, and rate-limit failure states. Stripe handoff success and
  tip/gift/unlock retry breadth remain open.
...
broken. The remaining release risk is Stripe redirect success/handoff, tip/gift/unlock retry UX,
and the broader final browser pass that exercises Nodes together with the rest of the app.
...
| Nodes and calls honesty                          | `70%` | Nodes false-success and schema drift are materially fixed in targeted tests, wallet/shop query failures now render explicit retryable failure states in focused page tests, `apps/backend/test/cgraph_web/controllers/nodes_controller_test.exs` proves wallet balance, filtered transaction history, active bundle schema, canonical checkout contracts, and tip/gift insufficient-balance plus success response contracts, and `apps/web/e2e/nodes-wallet-shop.spec.ts` browser-verifies routed wallet/shop rendering plus wallet, transaction-history, bundle-load, empty-shop, checkout-failure, and user-triggered retry recovery states. Insufficient-balance unlock recovery routes to the canonical wallet shop, `apps/web/e2e/dm-media-composer.spec.ts` browser-verifies routed paid-file insufficient-balance, already-unlocked, and rate-limit states, `apps/web/e2e/nodes-profile-actions.spec.ts` browser-verifies routed profile tip/gift insufficient-balance and retry-to-success behavior, call-history callback is browser-verified, and incoming-call accept/end-call route behavior is browser-verified. Stripe handoff success behavior, unlock retry breadth, and deeper peer media-negotiation trust work are still open. |
...
Equal-weight release-readiness score: `73.8%`.
...
- [x] Recheck insufficient-balance, already-unlocked, self-gift, rate-limit, and remaining negative
      ...
      success and tip/gift/unlock retry UX.

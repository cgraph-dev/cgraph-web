...
| Nodes and calls honesty                          | `65%` | Nodes false-success and schema drift are materially fixed in targeted tests, wallet/shop query failures now render explicit retryable failure states in focused page tests, `apps/web/e2e/nodes-wallet-shop.spec.ts` browser-verifies routed wallet/shop rendering plus wallet, transaction-history, bundle-load, empty-shop, and checkout-failure states, backend checkout tests now prove canonical wallet return routes plus Stripe rejection and unknown-bundle errors, insufficient-balance unlock recovery routes to the canonical wallet shop, `apps/web/e2e/dm-media-composer.spec.ts` browser-verifies routed paid-file insufficient-balance, already-unlocked, and rate-limit states, component tests cover tip/gift/content-unlock self-gift/rate-limit/already-unlocked copy, call-history callback is browser-verified, and incoming-call accept/end-call route behavior is browser-verified. Stripe handoff success behavior, broader retry UX, and deeper peer media-negotiation trust work are still open.                                                                                                                                                                                                                                                                       |
...
Equal-weight release-readiness score: `73.3%`.

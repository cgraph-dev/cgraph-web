...
| Routed group parity                              | `64%` | Group fetch/send works after landing on a valid channel, routed text and file/photo sends are browser-verified, voice/video channel routes now mount a LiveKit-backed group room view, group settings/admin tabs are mounted, routed loaded-message search and group-level mute/unmute are browser-verified, the live message action menu is browser-verified for edit, delete, report, pin, and copy-link, group reply/reaction/thread behavior is browser-verified, and the pinned panel is browser-verified for fetch/unpin/state reconciliation. Remaining gaps are permission edge states, older-message search coverage, browser proof for admin paths, channel-level notification semantics, and voice/sticker/GIF media parity. |
...
Equal-weight release-readiness score: `61.5%`.

...
3. Routed DM message-state controls are materially wired. Mark-as-read runs after route history
   ...
   bubble renders Seen/read-receipt state. Remaining message-state risk is typing browser proof and
   final conversation-list browser verification.
...
3. Routed DM parity
   - Browser-verify the current typing emit, then finish GIF/sticker and call-flow parity.
   - Browser-verify the routed conversation-list menu, including the new per-chat Space move
     controls.
   - Keep file/photo/voice, pinned-message UI, guarded autoscroll, and search anchors green while
     bringing GIF/sticker send and call launch onto the routed surface.
...
Equal-weight release-readiness score: `45.4%`.

...
3. Routed DM message-state controls are materially wired. Mark-as-read runs after route history
   ...
   bubble renders Seen/read-receipt state. Remaining message-state risk is guarded scroll/unread
   jump, typing browser proof, and final conversation-list browser verification.
...
3. Routed DM parity
   - Browser-verify the current typing emit, then finish guarded scroll and unread-jump behavior.
...
| Routed DM parity                                 | `70%` | Basic DM open/send, typing emit, search anchors, mark-as-read, browser-verified read-receipt rendering, browser-verified file/photo and voice-note send, routed reply/search jump/edit/delete/forward/request/pin/pinned-panel behavior, and sidebar mark-read/archive are now wired. Remaining gaps are canonical surface convergence, GIF/sticker parity, guarded autoscroll/unread jump, remaining list-action browser verification, call-flow verification, and broader final browser verification.                                                                                                                        |
...
Equal-weight release-readiness score: `45.1%`.

Truthful rounded Goal 2 completion: `45%`.

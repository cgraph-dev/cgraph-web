...
| Group channel pinned messages                       | Partial | `pinned-messages-panel.tsx` can fetch and unpin pins, and the routed message menu now creates pins through `/api/v1/groups/:groupId/channels/:channelId/pins`. Pin creation from the live message menu is browser-verified by `apps/web/e2e/web-owner-uat.spec.ts`; pinned-panel unpin and permission-denied states are still pending.                                                                                                                                                                                                                              |
...
8. Group message actions are browser-verified on the routed text-channel surface: the live message
   ...
   pins endpoint, submits reports, and copies `?scrollTo=...` links. Remaining risk is
   permission-denied states, pinned-panel unpin proof, and richer media/GIF/voice parity.
...
4. Routed group parity
   ...
   - Keep the browser-verified group message action menu proof green, then bring groups to parity
     on voice, stickers, and GIFs.
...
| Routed group parity                              | `60%` | Group fetch/send works after landing on a valid channel, routed text and file/photo sends are browser-verified, voice/video channel routes now mount a LiveKit-backed group room view, group settings/admin tabs are mounted, routed loaded-message search and group-level mute/unmute are browser-verified, and the live message action menu is browser-verified for edit, delete, report, pin, and copy-link. Remaining gaps are permission edge states, older-message search coverage, browser proof for admin/pinned-panel unpin paths, channel-level notification semantics, and voice/sticker/GIF media parity.                                                                                   |
...
Equal-weight release-readiness score: `61.1%`.

Truthful rounded Goal 2 completion: `61%`.

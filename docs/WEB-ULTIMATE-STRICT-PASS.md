...
| Routed group parity                              | `75%` | Group fetch/send works after landing on a valid channel, routed text and file/photo sends are browser-verified, routed GIF/sticker/voice sends are browser-verified, voice/video channel routes now mount a LiveKit-backed group room view, routed settings overview save, invite creation, member role assignment, role-tab render, channel creation, owner/non-admin settings permission gating, loaded-message search, group-level settings notifications, and channel-level mute/unmute are browser-verified. The live message action menu is browser-verified for edit, delete, report, pin, and copy-link, group reply/reaction/thread behavior is browser-verified, and the pinned panel is browser-verified for fetch/unpin/state reconciliation. Remaining gaps are endpoint-level permission-denied copy, stronger group scroll behavior, older-message search coverage, and deeper role CRUD/reorder proof.                                                                                                                                                            |
...
Equal-weight release-readiness score: `69.1%`.
...
- [ ] Finish group permission-edge proof, stronger group scroll behavior, older-message search, and
      deeper role CRUD/reorder. Channel-level notification semantics are now browser-verified
      through `/api/v1/notification-preferences/channel/:channelId`.

...
| Social hub route completeness                       | Partial | `social.tsx` fetches real friends, requests, notifications, and discovery results, renders a real main pane, and now has focused browser proof that selected notification, discover, and friend-request actions run through the routed stores. Remaining risk is broader friend action parity across Social and profile pages, not a placeholder center pane.                                                                                                                                                                                                                                                                                      |
...
3. The Social Hub is still incomplete, but it is no longer a placeholder shell. Friends,
   notifications, and discover all fetch real store data, the route renders a real main pane, and
   focused browser proof now covers selected notification deep links, discover group join/open, and
   main-pane friend-request accept. The remaining problems are broader action parity and final
   social/profile regression coverage.
...
5. Social discover routes forums by slug when available, users to profiles, and groups with backend
   ...
   Deeper Social/profile action parity remains open.
...
6. Account deletion now uses the right backend contract and exposes grace-period recovery on web.
   ...
   commit `93febe9` now browser-verifies the mounted cancellation and scheduling lifecycle, including
   the password payload and follow-up auth logout side effect.
...
| Settings, privacy, and customization convergence | `80%` | The settings route and panels are real, preference bootstrap has one owner, section panels are gated until bootstrap is ready, the runtime-neutral settings/defaults contract now lives in `packages/shared-types/src/settings.ts`, selective privacy has shared types/API/backend/web coverage, Calls/Stickers reset now saves through the settings API, customization/theme server patches now sync over the user channel, identity customization inventory/equipped state now comes from backend inventory with backend rejection of unowned saves, and `apps/web/e2e/settings-preference-sync.spec.ts` verifies privacy reload plus settings/customization/theme live-sync behavior on mounted routes. Remaining risk is real multi-tab/device socket validation and broader final browser coverage.                                                                                                                                                                                                                                   |
...
| Social, discovery, and notification destinations | `75%` | Data loads into the route, the Social Hub has a real main pane, notifications preserve action destinations, forums route by slug, groups with channel metadata route to mounted channel destinations, and unjoined group results run a real join action. `apps/web/e2e/social-main-pane.spec.ts` now browser-verifies notification mark-read plus deep-link navigation, discover search plus group join/open, and main-pane friend-request accept. Broader friend action parity across Social and profile pages remains.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| Final release validation                         | `0%`  | The final strict browser pass has not been rerun after the release-path fixes.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |

Equal-weight release-readiness score: `67.3%`.

Truthful rounded Goal 2 completion: `67%`.

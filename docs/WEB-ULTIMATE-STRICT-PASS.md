...
- settings hydration, privacy reload persistence, same-origin live sync, separate browser-profile
  server-event application, customization sync, and app-shell theme sync now have routed browser
  proof; physical second-device transport validation and broader final regression remain open
...
4. Settings and privacy ownership is closer to the Telegram model after the 2026-05-15 selective
  ...
  ready. Routed settings reload, live sync, same-origin cross-tab sync, and separate browser-profile
  server-event application are browser-verified; the remaining settings risk is physical
  second-device transport validation and broader final regression.
...
| --------------------------------------------------- | ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Cloud DM creation/opening                           | Ready   | `apps/web/src/pages/messages/messages/messages.tsx` still owns the route shell, `?userId=` handoff, conversation selection, and sidebar actions, but the opened Cloud Chat conversation now renders through shared `modules/chat/components/conversation-surface.tsx` and binds backend data, socket lifecycle, voice/file uploads, paid-file metadata, request state, read state, typing, message send, and routed action composition through `modules/chat/controllers/cloud-conversation`. The page now consumes the controller as a view-level slot source instead of composing the chat API contract itself. Focused proof lives in `apps/web/src/modules/chat/controllers/cloud-conversation/__tests__/cloud-conversation-owner.test.ts`, with the moved direct-call and voice-upload contract tests beside the controller.                                                                                                                                                                                                                                                                          |
...
| Direct call launch from DM route                    | Ready   | `conversation-header.tsx` routes voice/video buttons through `getDirectCallRoute(...)` to `/call/:recipientId/:callType`; `apps/web/e2e/dm-media-composer.spec.ts` browser-verifies live DM-header audio/video launch, and `apps/web/e2e/web-owner-uat.spec.ts` verifies the manual call route mounts controls. Incoming-call accept and end-call navigation are now verified separately, and `apps/web/src/lib/webrtc/__tests__/peerConnection.test.ts` now includes a linked two-peer signaling harness. Deeper production-like media negotiation remains tracked under final call-flow coverage.                                                                                                                                                                                                                                                                                                                                                                                 |
...
| Auth route-set parity coverage                      | Partial | The matrix below now browser-verifies routed email login with 2FA, registration, forgot-password, reset-password including invalid/expired/reused-token recovery, verify-email token/resend, QR session creation plus stale-code expiry/retry, existing-user phone login, new-user phone registration, OTP resend, call fallback, registration-lock, native-device-required recovery, onboarding, and destructive account lifecycle scheduling/cancellation. Remaining risk is real provider delivery, paired QR approval, and production-provider account validation.                                                                                                                                                                                                                                                                                                                                     |
| Email/password login                                | Partial | `useLoginForm.ts` calls `useAuthStore.login(...)`, supports the 2FA handoff through `verifyLoginTwoFactor(...)`, navigates to `/messages`, and the routed 2FA login path is browser-verified by `apps/web/e2e/auth-account-routes.spec.ts`. Remaining risk is real provider/session edge validation.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
- upload antivirus can still run metadata-only in production until a real ClamAV service is
  ...
  and production backend readiness now exposes the active scanner mode instead of hiding it
...
4. Routed DM call launch is now proven from the live header, and incoming-call accept now hands off
   ...
   Deeper peer media negotiation still needs browser verification against a production-like WebRTC
   session.
...
6. Call history trustworthiness is now route/contract/test aligned. The page fails honestly on API
   ...
   channel event owner; the remaining gap is a production-like two-client WebRTC browser session
   across real network conditions.
...
| Phase                                            | Score | Why                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ------------------------------------------------ | ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Messaging contract convergence                   | `84%` | Routed DM and group file/photo sends now use the same shared upload-first metadata contract from `packages/shared-types/src/media.ts`; both routed browser proofs are green, the unused alternate media-upload owner was removed, the routed Cloud Chat composer now consumes the shared `modules/chat` `MessageInput` contract for text, files, paid-file pricing, GIFs, stickers, voice notes, draft ownership, and typing events, the routed bubble delegates shared message rendering, media, pinned state, receipts, and actions to `MessageBubble`, the routed list delegates virtualized rendering plus id-based scroll requests to `MessageList`, chat UI defaults now live under `modules/chat/components/message-bubble/preferences.ts` with routed compatibility re-exports, and the opened DM frame now uses shared `ConversationSurface` slots for the stable header/message-scroll/composer/floating-control/request/pinned/modal layout. The 2026-05-28 controller refactor moves Cloud Chat data/socket/upload/action composition into `modules/chat/controllers/cloud-conversation`, leaving the page as a view-level slot consumer; `cloud-conversation-owner.test.ts`, direct-call routing, voice-upload tests, typecheck, the full Vitest run, and the 2026-05-29 combined browser route pass prove the route/contract boundary. Remaining convergence work is external proof and deeper production-like media validation.                                                                                                                                                                                                                                                     |
...
| Routed DM parity                                 | `89%` | Basic DM open/send, browser-verified typing emit, stable guarded search anchors, jump-to-latest behavior, mark-as-read, browser-verified read-receipt rendering, browser-verified file/photo, voice-note, GIF, and sticker send, browser-verified DM call-entry launch, routed reply/search jump/edit/delete/forward/request/pin/pinned-panel behavior, incoming-call accept/end-call route proof, browser-verified sidebar mark unread/read, pin/unpin, mute/unmute, archive/recover, and Space add/remove are now wired. The live Cloud Chat composer, message list, routed message bubble, and backend data/action controller now route through shared `modules/chat` contracts and are reverified by focused controller tests plus the full routed DM media/composer browser spec and 2026-05-29 combined route pass. Remaining gap is deeper production-like media-negotiation proof.                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
- [ ] Revalidate real cross-tab and cross-device socket delivery after store ownership cleanup.
      ...
      browser profiles consume the same server-shaped settings sync payload. Remaining: physical
      second-device socket delivery against production-like sessions.
...
- [ ] Finish upload hardening items that still block a real production claim. The 2026-05-23
      ...
      failure-policy state through health checks and `/ready`; production currently reports
      `antivirus:"metadata_only"`. This remains open until production has a provisioned, reachable
      ClamAV scanner.

...
| Web onboarding completion                           | Partial | `useOnboarding.ts` uploads avatar, updates `/api/v1/me`, saves notification preferences, and posts `/api/v1/me/onboarding/complete`. Skip now posts `/api/v1/onboarding/skip` before leaving the route, save/skip failures render a routed recovery error, and the app route guard now sends users to onboarding only after email verification is satisfied. Remaining risk is browser verification.                                                                                                                                                                                                                                    |
...
2. Onboarding no longer silently bypasses the server on skip, and it no longer depends on ad hoc
   ...
   `post-auth-redirect.ts` enforces verify-email before onboarding before `/messages`. The remaining

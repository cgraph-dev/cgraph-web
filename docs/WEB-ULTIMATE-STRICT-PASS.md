...
| Direct call launch from DM route                    | Ready   | `conversation-header.tsx` routes voice/video buttons through `getDirectCallRoute(...)` to `/call/:recipientId/:callType`; `apps/web/e2e/dm-media-composer.spec.ts` browser-verifies live DM-header audio/video launch, and `apps/web/e2e/web-owner-uat.spec.ts` verifies the manual call route mounts controls. Full media negotiation, incoming-call, and call-end behavior remain tracked under call-flow coverage.                                                                                                                         |
...
| Call launch coverage                                | Partial | The manual `/call/:recipientId/:callType` route, routed DM header launch, and call-history callback are browser-verified. Incoming call, media negotiation, and end-state behavior are not yet fully browser-verified route-owned end-to-end in this pass.                                                                                                                                                                                                                                                                                    |
...
3. Routed DM message-state controls are materially wired. Mark-as-read runs after route history
   fetch, backend message JSON carries read receipts as `metadata.readBy`, and the routed enhanced
   bubble renders Seen/read-receipt state. Remaining message-state risk is final conversation-list
   browser verification.

4. Routed DM call launch is now proven from the live header. A call route exists, the main DM header
   navigates to it for audio and video, and browser proof lives in
   `apps/web/e2e/dm-media-composer.spec.ts`. The end-to-end media/incoming/history call flow still
...
6. Call history is still not fully complete. The page now fails honestly on API failure, and the
   callback button is browser-verified from a real history row, but incoming/media/end-state call
   coverage and richer history-management behavior are still missing.
...
3. Routed DM parity
   - Keep the browser-verified typing and call-entry paths green while finishing GIF/sticker and
   ...
   - Keep file/photo/voice, pinned-message UI, guarded autoscroll, search anchors, and DM call
     launch green while bringing GIF/sticker send and full call-flow proof onto the routed surface.
...
7. Nodes and calls honesty
   - Browser-verify wallet/shop/tip/gift/unlock/checkout negative paths after the throwing client
     and schema fixes.
   - Finish call-flow browser verification, including incoming-call state and media/end-state
     behavior.
...
| Routed DM parity                                 | `74%` | Basic DM open/send, browser-verified typing emit, stable guarded search anchors, jump-to-latest behavior, mark-as-read, browser-verified read-receipt rendering, browser-verified file/photo and voice-note send, browser-verified DM call-entry launch, routed reply/search jump/edit/delete/forward/request/pin/pinned-panel behavior, and sidebar mark-read/archive are now wired. Remaining gaps are canonical surface convergence, GIF/sticker parity, remaining list-action browser verification, incoming/media/end-state call-flow verification, and broader final browser verification.                               |
...
| Nodes and calls honesty                          | `52%` | Nodes false-success and schema drift are materially fixed in targeted tests, and call-history callback is now browser-verified. Browser negative-path validation plus incoming/media/end-state call-flow trust work are still open.                                                                                                                                                                                                                                                                                                                                                                                            |
...
Equal-weight release-readiness score: `46.1%`.

Truthful rounded Goal 2 completion: `46%`.
...
- [ ] Browser-verify incoming-call flow and full media/end-state behavior.
...
- [ ] Verify incoming-call flow and full media/end-state behavior.

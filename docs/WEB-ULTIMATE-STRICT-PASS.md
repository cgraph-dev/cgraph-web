...
1. Conversation list management is release-closed for the routed live menu. The live message sidebar can search, open, mark
   ...
   lists. `apps/web/e2e/dm-media-composer.spec.ts` now browser-verifies mark unread/read,
   pin/unpin, mute/unmute, archive/recover, and Space add/remove from the live route.
...
3. Routed DM message-state controls are browser-proven for the audited route-owned state. Mark-as-read runs after route history
   fetch, backend message JSON carries read receipts as `metadata.readBy`, and the routed enhanced
   bubble renders Seen/read-receipt state. The live conversation-list menu is also verified for
   mark unread/read, pin/unpin, mute/unmute, archive/recover, and Space add/remove.
...
Equal-weight release-readiness score: `59.2%`.

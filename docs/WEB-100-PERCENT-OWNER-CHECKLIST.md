...
Current execution count after the preference live-sync bridge slice: 41 / 50 checklist items closed,
82.0% closed and 18.0% left. This is not 100% yet because final support-matrix validation, final
...
- [x] Routed DM voice-note send is real and browser-verified. Verified by
      `apps/web/e2e/dm-media-composer.spec.ts` on 2026-05-14.
- [x] Routed DM reply, edit, delete, forward, request, pin, and search-jump behavior are complete.
      Verified by `apps/web/e2e/dm-media-composer.spec.ts` on 2026-05-14.

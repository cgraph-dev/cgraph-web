import type { CanonicalUserIdentity, IdentityStatus } from '@cgraph/shared-types';

const IDENTITY_STATUSES: readonly IdentityStatus[] = [
  'online',
  'idle',
  'dnd',
  'offline',
...
function isIdentityStatus(value: string | null): value is IdentityStatus {
  return IDENTITY_STATUSES.some((status) => status === value);
}

function normalizeStatus(value: string | null): IdentityStatus {
...
export function canonicalIdentityFromApi(raw: Record<string, unknown>): CanonicalUserIdentity {
  ...
  return {
    ...
    status: normalizeStatus(firstString(records, ['status', 'presence'])),
